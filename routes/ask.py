...
def ask_ai():
    ...
    try:
        ...
        for pid in ranked_item_ids:
            ...
            # Prioritize subtitle (usually event locations), then label name (for map polygons),
            # then fallback to location or host.
            snippet = ""

            if matched_item.get("subtitle"):
                snippet = str(matched_item.get("subtitle")).strip()
            elif isinstance(matched_item.get("label"), dict) and matched_item["label"].get("name"):
